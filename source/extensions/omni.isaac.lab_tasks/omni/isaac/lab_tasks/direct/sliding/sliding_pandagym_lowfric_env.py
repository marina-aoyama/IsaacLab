...
@configclass
class SlidingPandaGymLowFricEnvCfg(DirectRLEnvCfg):
    # simulation
    ...
    # Start region
    max_pusher_posx_bound = 1.5  # the cart is reset if it exceeds that position [m]
    ...
    max_pusher_posy_bound = 0.15  # the cart is reset if it exceeds that position [m]
    min_pusher_posy_bound = -0.15  # the cart is reset if it exceeds that position [m]
    ...
    start_width = abs(max_pusher_posy_bound - min_pusher_posy_bound)
    ...
    markerstart1_cfg = VisualizationMarkersCfg(
        ...
        markers={
            "cube": sim_utils.CuboidCfg(
                size=(start_length, start_width, 0.01),
...
class SlidingPandaGymLowFricEnv(DirectRLEnvFeedback):
    cfg: SlidingPandaGymLowFricEnvCfg

    def __init__(self, cfg: SlidingPandaGymLowFricEnvCfg, render_mode: str | None = None, **kwargs):
    ...
    def _pre_physics_step(self, actions: torch.Tensor) -> None:
        ...
        self.actions[:, 0] = self.actions[:, 0] * 1.0
        self.actions[:, 1] = self.actions[:, 1] * 0.4
        # self.actions[self.actions[:,0] < -1.0] = -1.0
        # self.actions[self.actions[:,0] > 1.0] = 1.0
        # self.actions[self.actions[:,1] < -0.4] = -0.4
        # self.actions[self.actions[:,1] > 0.4] = 0.4
        # Clamp the values in the first dimension to be within [-1.0, 1.0]
        # self.actions[:, 0] = torch.where(self.actions[:, 0] < -100.0, -100.0, self.actions[:, 0])
        # self.actions[:, 0] = torch.where(self.actions[:, 0] > 100.0, 100.0, self.actions[:, 0])

        # # Clamp the values in the second dimension to be within [-0.4, 0.4]
        # self.actions[:, 1] = torch.where(self.actions[:, 1] < -100.0, -100.0, self.actions[:, 1])
        # self.actions[:, 1] = torch.where(self.actions[:, 1] > 100.0, 100.0, self.actions[:, 1])
        # actions[:, 1] = actions[:, 1].clamp(-0.4, 0.4)  # Limit for second dim
        # actions[:, 0] = actions[:, 0].clamp(-1.0, 1.0)  # Limit for first dim
        # self.actions[:, 1] = torch.clamp(actions[:,1], min=-10.4, max=10.4)
        # self.actions[:, 0] = torch.clamp(actions[:,0], min=-10.0, max=10.0)
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        cuboidpusher2_default_state[:, 7:] = torch.zeros_like(self.cuboidpusher2.data.default_root_state[env_ids, 7:])
        # cuboidpusher2_default_state[:, 7] = -1.0
        self.cuboidpusher2_state[env_ids] = cuboidpusher2_default_state.clone()
        self.cuboidpusher2.write_root_state_to_sim(cuboidpusher2_default_state, env_ids)
