...
def main():
    ...
    env._set_property_noise(prop_noise_std_dict)
    sensitivity_terminate = False
    # simulate environment
    while simulation_app.is_running() and sensitivity_terminate==False:
        ...
        with torch.inference_mode():
            ...
            import time
            if infos["log_eval"]["all_env_total_num"] >= 50000: 
                curr_success_rate = (infos["log_eval"]["all_env_success_num"]/infos["log_eval"]["all_env_total_num"])*100
                if gt_success_rate is None: 
                    gt_success_rate = curr_success_rate
                    print("GT success rate")
                    print(gt_success_rate)
                    print("Groundtruth success rate recorded")
                    time.sleep(3)
                    env._reset_trial_count()
                else: 
                    if curr_success_rate>(gt_success_rate-5): 
                        prop_noise_std_dict = {curr_prop: prop_noise_std_dict[curr_prop]+0.001}
                        env._set_property_noise(prop_noise_std_dict)
                        print("GT success rate")
                        print(gt_success_rate)
                        ...
                        print("Current success rate")
                        print(curr_success_rate)
                        print("Sensitivity terminated")
                        print("Estimation threshold (" + curr_prop + "): " + str(prop_noise_std_dict[curr_prop]))
                        time.sleep(3)
